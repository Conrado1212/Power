Function Get-SpaceAge() {
    ...
    $xd = 31557600 
    $year = @{Mercury =	0.2408467; Venus = 0.61519726; Earth = 1.0; Mars = 1.8808158; Jupiter=11.862615; Saturn= 29.447498;Uranus= 84.016846; Neptune= 164.79132}
    if($year.ContainsKey($Planet)){
      return  [Math]::Round($Seconds/($xd*$year[$Planet]),2)
    }elseif ([string]::IsNullOrEmpty($Planet)){
        return  [Math]::Round($Seconds/($xd*$year["Earth"]),2)
    }else{
...
#Get-SpaceAge -Seconds 1000000000 -Planet Earth

#Get-SpaceAge -Seconds 1000000000

Get-SpaceAge -Seconds 1000000000 -Planet Pluto