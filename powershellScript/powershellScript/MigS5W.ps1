...
Write-MessageLog '[Line 153] Groups that are only in environment1' -Color Red
...
if($onlyIn1env.count -gt 0){
   ...
   # $groupMemberenv1  = Get-Member-From-Group -baseUrl $baseUrl -clientId $CLIENT_ID -clientSecret $CLIENT_SECRET  -checkGroupMember $onlyIn1env
    if($userInput -eq "Y"){
