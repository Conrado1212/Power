...
function Get-DataFromExcel{
    ...
    $excel = New-Object -ComObject Excel.Application
     if([System.IO.Path]::GetExtension($path) -ne ".xlsx"){
        Write-MessageLog "The file must have the xlsx extension" 
        exit 1
    }
        ...
        while ($sheet.Cells.Item($row, 1).Text -ne "") {
            try{
                ...
                Write-MessageLog "[Line 71] E in row $row : $_" -Level "WARNING"
            }
            $row++;
        }
        ...
        while ($sheet.Cells.Item($row, 1).Text -ne "") {
            try{
               ...
               if (-not $groups.ContainsKey($groupId)) {
                ...
                Write-Warning "[Line 71] E in row $row : $_" -Level "WARNING"
            }
            $row++;
        }
        $workbook.Close($false)
...
if([string]::IsNullOrWhiteSpace($path)){
    Write-MessageLog "the Path variable cannot be empty" 
    exit 1
}else{
 $response =  Get-DataFromExcel -path $path
 Write-MessageLog "[line 128] Number of groups in the excelSheet: $($response.Count)" 
}
...
if($onlyIn1env.count -gt 0){
 ...
 if ($missingGroups.Count -gt 0) {
    ...
    Write-MessageLog  "No difference"
 }
...
function Get-Member-From-Group{
 ...
 if($userInputM -eq "Y"){
      $groupMemberenv1  = Get-MemberDataFromExcel -path $path
 ...
 function Compare-Group-Members{
     ...
     foreach($group1 in $groupMemberenv1){
         ...
         if(-not $group2){
             Write-MessageLog "[Line 294] Group $groupId not found in second environment" 
             continue
         }
      
        $members1 = $group1.members
       
        Write-MessageLog  "[Line 300] Members in ev1 for group $groupID" 
       
         if($members1.count -eq 0){
             Write-MessageLog  "[Line 303] No members" 
         }else{
           
            $members1 | foreach-Object {Write-MessageLog " - '$($_.bpsId)'" }
         
         }

         $members2 = $group2.members

         Write-MessageLog  "[Line 312] Members in ev2 for group $groupID" 
         if($members2.count -eq 0){
             Write-MessageLog  "[Line 314] No members"
         }else{
             $members2 | foreach-Object {Write-MessageLog "[Line 316] - '$($_.bpsId)'" }
        ...
        $onlyIn1env = $normalizedMembers1 | Where-Object {$_ -notin $normalizedMembers2}
         ...
         if($onlyIn1env.count -eq 0 -and $onlyIn2env.count -eq 0){
             Write-MessageLog "[Line 331] Group $groupID has identical members" 
         }else{
             Write-MessageLog "`n [Line 333] Differences in $groupID"  -Color Magenta
        
              if($onlyIn1env.count -gt 0 ){  
                 Write-MessageLog "[Line 336] - Only in Env1:" 
                 forEach($bpsId in $onlyIn1env){
             

                     $member =  $members1 | Where-Object {$_.bpsId.ToString().Trim().ToLower() -eq $bpsId }  
                     if($member){
                Write-MessageLog "   - [$($member.bpsId)] (env1 only)"  -Color Yellow
    ...
    Write-MessageLog " Summary of Differences"  -Color Cyan
     Write-MessageLog "Env1 unique members: $($result.Count)" 
     $result | Group-Object bpsGroupId | ForEach-Object{
        $group = $_
        Write-MessageLog "[Line 477] Group: $($group.Name)"  -Color Cyan
     }
     foreach($member in $result){
        Write-MessageLog "[Line 480] [BpsUserId: $($group.bpsMemberId), Name: $($group.MemberName), Email: $($group.MemberEmail), Type: $($group.MemberType), ENV: $($group.environment)]" 
     }
     Write-MessageLog "Env2 unique members: $($result2.Count)" 
     $result2 | Group-Object bpsGroupId | ForEach-Object{
        $group = $_
        Write-MessageLog "[Line 485] Group: $($group.Name)"  -Color Cyan
     }
     foreach($member in $result2){
        Write-MessageLog "[Line 488] [BpsUserId: $($group.bpsMemberId), Name: $($group.MemberName), Email: $($group.MemberEmail), Type: $($group.MemberType), ENV: $($group.environment)]" 
     }
     return @{
         env1 = $result
         env2 = $result2
