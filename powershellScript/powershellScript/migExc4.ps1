     ...
     if([System.IO.Path]::GetExtension($path) -ne ".xlsx"){
        ...
        foreach($groupId in $groups.Keys){
            $result += [PSCustomObject]@{
                bpsId = $groupId
                name = $groupMeta[$groupId].name
                email = $groupMeta[$groupId].email
 ...
 if($group.members){
        foreach($member in $group.members){
             ...
             if(-not $exitsMember -and $member.type -eq 'User'){
                 if($AutoApprove -or (Read-Host "[Line 512] Do you want to create this user $memberId as bpsUser? (Y/N)") -eq "Y"){
...
if($onlyIn1env.count -gt 0){
    Write-MessageLog "[Line 187] Number of missing groups: $($onlyIn1env.Count)" 
     if($AutoApprove -or(Read-Host "[Line 188] Do you want to create the missing groups in second environment? (Y/N)") -eq "Y"){
  
    $ddd = Get-MemberDataFromExcel -path $path
$onlyIds = $onlyIn1env | Select-Object -ExpandProperty bpsId
$filteredGroups = $ddd | Where-Object { $_.bpsId -in $onlyIds }
    
         New-Group -baseUrl $baseUrl1 -clientId $CLIENT_ID1 -clientSecret $CLIENT_SECRET1  -groupToCreate $filteredGroups
         $maxRetries = 5
         $retryCount = 0
     do{
    $response1 = Get-Group -baseUrl $baseUrl1 -clientId $CLIENT_ID1 -clientSecret $CLIENT_SECRET1
 ...
 if ($missingGroups.Count -gt 0) {
    ...
    Write-MessageLog  "No difference" -Color Green
 }
...
if($userInputM -eq 'Y'){
if($AutoApprove -or (Read-Host "[Line 565] Do you want to add missing member to group? (Y/N)") -eq "Y"){
 ...
 $onlyIn2env = $onlyIn2env | Sort-Object bpsId -Unique
  ...
  function Remove-Member{
   ...
   if($env2.count -gt 0){
    if($AutoApprove -or (Read-Host "[Line 665] Do you want to remove excess users from bpsGroup in environment2 ? (Y/N)") -eq "Y"){
