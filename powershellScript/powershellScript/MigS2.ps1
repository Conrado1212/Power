...
function Compare-Group-Members{
    ...
    foreach($group1 in $groupMemberenv1){
       ...
       Write-MessageLog  "[Line 300] Members in ev1 for group $groupID" -Level "INFO"
        ...
        if($onlyIn1env.count -eq 0 -and $onlyIn2env.count -eq 0){
            ...
            Write-Host "`n [Line 333] Differences in $groupID" -ForegroundColor Magenta
             ...
             if($onlyIn1env.count -gt 0 ){  
                ...
                forEach($bpsId in $onlyIn1env){
                    ...
                    if($member){
               Write-Host "   - [$($member.bpsId)] (env1 only)" -ForegroundColor Yellow
            ...
            forEach($bpsId in $onlyIn2env){
                    ...
                    if($member){
                         Write-Host "   - [$($member.bpsId)] (env2 only)" -ForegroundColor Red
                     $name = if ($member.name) { $member.name } else { "?" }
                        $email = if ($member.email) { $member.email } else { "?" }
                        $type = if ($member.type) { $member.type } else { "?" }
    ...
    Write-MessageLog "Env1 unique members: $($result.Count)" -Level "INFO"
    Write-MessageLog "Env2 unique members: $($result2.Count)" -Level "INFO"
    return @{
        env1 = $result
        env2 = $result2
     }
...
Remove-GroupBPS -baseUrl $baseUrl1 -clientId $CLIENT_ID1 -clientSecret $CLIENT_SECRET1 -groupToDelete $onlyIn2env
  ...
  function Remove-Member{
    ...
    foreach($group in $groupMember){
        ...
        try{
            ...
            Write-MessageLog "Removed member $memberId from group $groupID" -Level "INFO"
        }catch{
  ...
  if($env2.count -gt 0){
    ...
    if($userInputR -eq "Y"){
        Write-MessageLog "Starting users deletion for $($env2.Count) unique users" -Level "INFO"
        Remove-Member -baseUrl $baseUrl1 -clientId $CLIENT_ID1 -clientSecret $CLIENT_SECRET1 -groupMember $env2
    }else{
         Write-MessageLog "[Line 670] Users deletion skipped" -Level "INFO"
    }
  }
