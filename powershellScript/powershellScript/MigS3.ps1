...
Write-MessageLog "[Line 141] Number of groups other than in environment2: $($onlyIn1env.Count)" 
    ...
    Write-MessageLog "[Line 148] No groups other than on  environment2 "  -Color Green
}
...
function Compare-Group-Members{
   ...
   # Write-MessageLog "Env1 unique members to Add: $($result.Count)"  -Color Green
   # $result | Group-Object bpsGroupId | ForEach-Object{
    #     $group = $_
    #     Write-MessageLog "[Line 407] Group: $($group.Name)"  -Color Cyan
     #     foreach($member in $result){
      #  Write-MessageLog " -[Line413] [BpsUserId: $($member.bpsMemberId), Name: $($member.MemberName), Email: $($member.MemberEmail), Type: $($member.MemberType), ENV: $($member.environment)]" 
# }
   # }
   
 #   Write-MessageLog "Env2 unique members to delete: $($result2.Count)"  -Color Red
  #  $result2 | Group-Object bpsGroupId | ForEach-Object{
  #  $group = $_
  #   Write-MessageLog "[Line 407] Group: $($group.Name)"   -Color Cyan
  #   foreach($member in $group.Group){
 #        Write-MessageLog " -[Line 413] [BpsUserId: $($member.bpsMemberId), Name: $($member.MemberName), Email: $($member.MemberEmail), Type: $($member.MemberType), ENV: $($member.environment)]" 
#     }
  #  }
    return @{
        env1 = $result
