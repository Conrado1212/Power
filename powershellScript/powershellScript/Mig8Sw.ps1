    ...
    if($group.members){
        foreach($member in $group.members){
             ...
             if(-not $exitsMember -and $member.type -eq 'User'){
                if($AutoApprove -or(Read-Host "[Line 512] Do you want to create this user $($member.bpsId) as bpsUser? (Y/N)") -eq "Y"){
...
#          $exitsMember = Get-User -baseUrl $baseUrl1 -clientId $CLIENT_ID1 -clientSecret $CLIENT_SECRET1 -user $member.bpsId
#          if(-not $exitsMember -and $member.type -eq 'User'){
#             Write-Host "[line580]   czy chcesz dodac "
#          }
#            $members += @{bpsId = $member.bpsId}
#        }
#    }
...
$onlyIn2env | Format-Table bpsId, name, email
if($AutoApprove -or (Read-Host "[Line 609] Do you want to remove excess groups from environment2 ? (Y/N)") -eq "Y"){
