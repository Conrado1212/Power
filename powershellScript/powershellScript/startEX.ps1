...
function Get-DataFromExcel{
    $excel = New-Object -ComObject Excel.Application
        $workbook = $excel.Workbooks.Open("C:\Users\k.krawczyk\Desktop\xd\fizyk\powershellScript\Book1.xlsx")
        ...
        for ($i = 1; $i -le $lastRow; $i++) {
            try{
                $status  = $sheet.Cells.Item($i,3).Text

                if($status -eq "AKTYWNA"){
                    $dane += [PSCustomObject]@{
                        id = $i
        ...
        $body =@{
          ...
          #$testBody = [System.Text.Encoding]::UTF8.GetBytes($body)
           ...
           $response =  Invoke-RestMethod -Method Post -Uri $new -Body $Body  -Headers $headers 
            Write-Host "[Line 124] Instance created for " $item.id 
            Write-Host "[Line 125] Response: $($response | ConvertTo-Json -Depth 5)"
        }catch{
        Write-Error "[Line 126] Error : $_"
        return $null
    }
    }  
